...
import { CurrencyProvider, DEFAULT_ACCOUNT_CURRENCY, useCurrency } from '@/lib/currency';
import { normalizeExecutableParameters } from '@workspace/api-zod';
import '@/index.css';

const queryClient = new QueryClient();
const conceptSupportStatus = (concept: TradingConcept) => concept.canonicalStatus === "executable"
  && normalizeExecutableParameters(concept.name, undefined)
  ? "supported"
  : concept.isBuiltIn ? "unexecutable" : "review";
const navGroups = [
...
function backtestErrorCopy(value: unknown) {
  ...
  if (/Dukascopy rejected|available cached .* candles cover|no cached .* candles are available|requested range:/i.test(message)) return message;
...
function backtestCandleSummary(backtest: Backtest) {
  if (backtest.status === "failed" && backtest.progress.candlesDownloaded > 0) {
    return `${backtest.progress.candlesDownloaded.toLocaleString()} candles downloaded · ${backtest.candlesProcessed.toLocaleString()} processed`;
  }
  return `${backtest.candlesProcessed.toLocaleString()} candles · ${backtest.tradeCount} simulated trades`;
}
